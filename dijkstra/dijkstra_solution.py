...
def dijkstra(graph, start_node):
    ...
    while(len(visited_nodes) < len(graph.nodes)):
        distance_to_me = estimated_distances[current_node]
        for edge in graph.get_edges(current_node):
            if edge.to in visited_nodes: continue
            if distance_to_me + edge.weight < estimated_distances[edge.to]:
                estimated_distances[edge.to] = distance_to_me + edge.weight

        visited_nodes.add(current_node)
        lowest_estimate = (None, INFINITY)
        for node in graph.nodes - visited_nodes:
            if estimated_distances[node] < lowest_estimate[1]:
                lowest_estimate = (node, estimated_distances[node])
        if lowest_estimate[0] is None: break
        current_node = lowest_estimate[0]
